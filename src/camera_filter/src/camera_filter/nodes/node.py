...
class CameraNode:
    ...
    def callback_camera(self, image):
        ...
        #prepreka(zuta)
        filteredImage = self.cf_class.filter_image(cutImage, np.array([25, 100, 100]), np.array([35, 255, 255]), 'yellow')
        self.matrix = self.cf_class.populate_matrix(filteredImage, self.matrix, TableFieldType.FIELD_OBSTACLE.value)
        
        #cilj1(crvena)
        filteredImage = self.cf_class.filter_image(cutImage, np.array([0, 100, 100]), np.array([15, 255, 255]), 'red')
        self.matrix = self.cf_class.populate_matrix(filteredImage, self.matrix, TableFieldType.FIELD_ENDING1.value)
         
        #cilj2(ljubicasta)
        filteredImage = self.cf_class.filter_image(cutImage, np.array([90, 100, 100]), np.array([115, 255, 255]), 'purple')
        self.matrix = self.cf_class.populate_matrix(filteredImage, self.matrix, TableFieldType.FIELD_ENDING2.value) 
              
        #robot(plava)
        filteredImage = self.cf_class.filter_image(cutImage, np.array([120, 100, 100]), np.array([130, 255, 255]), 'blue')
        self.matrix = self.cf_class.populate_matrix(filteredImage, self.matrix, TableFieldType.FIELD_ROBOT.value)
        
        #objekat(senf zuta)
        filteredImage = self.cf_class.filter_image(cutImage, np.array([20,100,100]), np.array([25, 255, 255]), 'mustard')
        ...
        np.savetxt('/home/crumb/MEHATRONIKA/project_ws/src/camera_filter/src/camera_filter/nodes/matricaa.txt', downscaled_matrix, fmt='%.1d')
